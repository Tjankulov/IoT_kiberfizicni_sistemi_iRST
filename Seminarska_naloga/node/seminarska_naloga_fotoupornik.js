// Povezava_klient_strežnik_modul esp32_fotoupornik
...
app.get('/', function(req, res){
    res.sendFile(path.join(__dirname + '/seminarska_naloga_fotoupornik.html'));
});
...
wss2.broadcast = function broadcast(data){ // broadcast za esp32 kliente
...
// ***************************************************************
// Brskalnik (klient) priključen na wss1, vrata 8888
// ***************************************************************
wss1.on('connection', function(ws, req){
    console.log("Klient - Brskalnik se je povezal z dvosmerno povezavo prek vrat 8888.");
    ws.on('message', function(msgString){ // sporočilo dobimo na stran strežnika kot string, t.j. msgString
        ...
        switch(msg.tipSporočila){
            case "LED":
                funkcijaLED(msg); // v primeru, da je tip sporočila iz klienta "LED" poženemo funkcijo funkcijaLED
            break;
        }
...
wss2.on('connection', function(ws, req){
    ...
    ws.on('message', function(msgString){ // sporočilo dobimo na stran strežnika kot string, t.j. msgString
        ...
        switch(msg.tipSporočila){
            case "fotoupornik":
                funkcijaFotoupornik(msg); // v primeru, da je tip sporočila iz esp32 modula fotoupornik, poženemo funkcijo funkcijaLED
            break;

            case "LED":
                funkcijaPovratnaLed(msg);
            break;
        }
...
{
    wss2.broadcast(JSON.stringify(msg)); // pošljemo sporočilo ESP32 modulu na portu 8811
}

function funkcijaPovratnaLed(msg) {
    wss1.broadcast(JSON.stringify(msg));
}

function funkcijaFotoupornik(msg) // če je tip sporočila "fotoupornik"
{ 
    wss1.broadcast(JSON.stringify(msg)); // "broad-castamo" po vseh brskalnik klientih - JSON sporočilo posredujemo naprej, tokrat vsem brskalnik klientom   
}