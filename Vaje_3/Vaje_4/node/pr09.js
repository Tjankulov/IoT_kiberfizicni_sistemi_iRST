// Povezava_klient_strežnik_modul esp32_upravljanje
...
app.get("/", (req, res) => {
    res.sendFile(join(_dirname, "pr09.html")); // pošlje datoteko pr07.html kot odgovor na zahtevo ("request - response")
});


// Funkcija za posredovanje sporočil vsem aktivnim klientom na vtičniku wss2 - port 8811
wss2.broadcast = function broadcast(data){ // to je broadcast za esp32 kliente
    wss2.clients.forEach(function each(client){ // gremo po vseh klientih in za vsakega klienta
        if(client.readyState === WebSocket.OPEN){  // če je povezava odprta
